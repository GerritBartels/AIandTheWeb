...
class Recommender(tf.keras.Model):
    ...
    def __init__(self, hidden_size: int, embedding_dim:int, dropout: float, num_users:int , num_movies: int) -> None:
...
def train_model(hidden_size, embedding_dim, dropout, batch_size, learning_rate, epochs, num_users, num_movies) -> None:
    ...
    model = Recommender(hidden_size=hidden_size, embedding_dim=embedding_dim, dropout=dropout, num_users=num_users, num_movies=num_movies)
    ...
    model.fit(train_data.shuffle(100000).batch(batch_size), epochs=epochs, validation_data=test_data.batch(batch_size))
